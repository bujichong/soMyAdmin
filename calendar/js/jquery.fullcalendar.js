...
(function ($) {
	...
	var favTpl = '<font color="#000000">{fav}</font>';
	...
	function show(target) {
		...
		$(target).find('.calendar-title span').html(opts.year + '年 ' +opts.months[opts.month - 1] + ' ' +  yDisplay + '年 农历 ' + cyclical(opts.year - 1900 + 36) + '年 【' + Animals[(opts.year - 4) % 12] + '年】');
		...
		for (var i = 0; i < weeks.length; i++) {
			...
			for (var j = 0; j < week.length; j++) {
				var day = week[j];
				var dayHtml = '<font style="font-size:14px;font-weight:bold;">' + day[2];
				var info = null;
				if (opts.month == day[1]) {
					info = currentCa[day[2] - 1];
					if (info.color) {
						var color = 'color="' + info.color + '"';
						dayHtml = '<font style="font-size:14px;font-weight:bold;" ' + color + '>' + day[2];
					}
					dayHtml += '</font>';
					if (opts.lunarDay) {
						dayHtml += "<br/>";
	...
	//增加样式
	$('head').append('<style>.fullcalendar-day {border-left: 1px solid #ddd;border-top: 1px solid #ddd;} '
		 + '.calendar-hover{}.fullcalendar-body th{padding: 5px;height:20px;}'
		 + '.fullcalendar-detail{border-radius:0 0 2px 2px;position: absolute; z-index: 10; background-color: rgb(0, 0, 102); display: none; opacity: 0.9; padding: 5px; text-align: right;filter: Alpha(opacity=90);width:150px;box-shadow:1px 1px 6px #999;-webkit-box-shadow:1px 1px 6px #999;-moz-box-shadow:1px 1px 6px #999;}'
		 + '.fullcalendar-detail div{background:#CCFFCC;margin-top: 3px;font-size: 12px;padding: 2px;}'
		 + '.fullcalendar-detail .lunarFestival{color: red;}.fullcalendar-detail .solarFestival{color: blue;} .calendar-selected {border:1px solid #ff6722;}</style>');
